...
const BOT_NAME_FANCY = 'SAYURA MD MINI BOT';
/*
const config = {
  AUTO_AI: true,           // true => auto AI reply ON, false => OFF
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/Lqv01dV0ZhS8MCoKaACKVj?mode=gi_t',
  RCD_IMAGE_PATH: 'https://raw.githubusercontent.com/sayuaradark-pixel/Botditels/refs/heads/main/file_00000000ec9071f8a50f1fa572f75f52.png',
  NEWSLETTER_JID: '120363322195409882@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94743826406',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbCMMJqKbYMVgMXwHg44',
  BOT_NAME: 'SAYURA MD MINI BOT',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'SAYURA MIHIRANGA',
  IMAGE_PATH: 'https://raw.githubusercontent.com/sayuaradark-pixel/Botditels/refs/heads/main/file_00000000ec9071f8a50f1fa572f75f52.png',
  BOT_FOOTER: '*㋛ SAYURA MD MINI BY SAYURA*',
  BUTTON_IMAGES: { ALIVE: 'https://raw.githubusercontent.com/sayuaradark-pixel/Botditels/refs/heads/main/file_00000000ec9071f8a50f1fa572f75f52.png' }
};
*/


/* ===== ADD THIS HERE (plugin loader) ===== */
...
async function sendOTP(socket, number, otp) {
  ...
  const message = formatMessage(`*🔐 Otp Veryfication — ${BOT_NAME_FANCY}*`, `*𝐘𝙾𝚄𝚁 𝐎𝚃𝙿 𝐅𝙾𝚁 𝐂𝙾𝙽𝙵𝙸𝙶 𝐔𝙿𝙳𝙰𝚃𝙴 𝐈𝚂:* *${otp}*\n𝐓𝙷𝙸𝚂 𝐎𝚃𝙿 𝐖𝙸𝙻𝙻 𝐄𝚇𝙿𝙸𝚁𝙴 𝐈𝙽 5 𝐌𝙸𝙽𝚄𝚃𝙴𝚂.\n\n*𝐍𝚄𝙼𝙱𝙴𝚁:* ${number}`, BOT_NAME_FANCY);
...
async function handleMessage(msg) {
    try {
        ...
        if (!text || msg.key.fromMe) return; // ignore own messages

        // AUTO_AI check
        if (!config.AUTO_AI) return; // false නම් skip කරන්න

        const botName = config.BOT_NAME || 'SHALA MD MINI BOT';
...
"මාව හැදුවේ සයුර මිහිරන්ග විසින්" කියන්න.
...
`;
        ...
        // Send reply only if AUTO_AI is ON
        if (config.AUTO_AI) {
...
END:VCARD`
    ...
    try {
        ...
        if (!data || !data.code) {
            return await socket.sendMessage(sender, {
                text: '❌ Pair code generate failed.'
            }, { quoted: shala });
        }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'SHALA-MD-MINI'}`); } catch(e) {}
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
